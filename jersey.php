...
<img class="bg" src="fallback.jpg" alt="UEFA">
    ...
    <div class="navbar">
      ...
      <div class="container">
        <p style="color: white;">Select Jersey No.</p>
        <form method="post">
  
  <select id="Jersey" name ="Jersey" style="margin:10px;">
    <option disabled selected>-- Select Jersey No.--</option>
    <?php
      ...
      $con = mysqli_connect($servername, $username, $password,$db);
        ...
        $records = mysqli_query($con, "SELECT Jersey From player_profile");  // Use select query here 

        while($data = mysqli_fetch_array($records))
        {
            echo "<option value='". $data['Jersey'] ."'>" .$data['Jersey'] ."</option>";  // displaying data in option menu
        }	
    ?>  
    </select>
...
$sql="CALL `playerjersey`('$name');";
$result = mysqli_query($con,$sql);
    if ($result !== false && $result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
            echo " <br> Player Name: " . $row["Name"]." <br> &nbsp &nbsp  &nbsp- Team Name: " . $row["Team_Name"].  "<br>";
        }
    } else {
        echo "0 results";
    }  
