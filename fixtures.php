<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>UEFA Champions league</title>
    <link href="https://fonts.googleapis.com/css?family=Roboto|Sriracha&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="style.css">
</head>
<style>
.league_stand{
    color: white;
    text-align: center;
    font-family: Verdana, Geneva, Tahoma, sans-serif;
    
  }
  .bg{
  width: 100%;
  position: absolute;
  z-index: -1;
} 
    body {
    margin: 0;
    padding: 0;
    font-family: sans-serif;
    background: url() no-repeat;
    background-size: cover;
    }
    .navbar {
    overflow: hidden; /* Hide overflow */
    background-color: transparent ; /* Dark background color */
    font-family: sans-serif;
    font-weight: bold;
  }
  
  /* Style the navigation bar links */
  .navbar a {
    float: left; /* Make sure that the links stay side-by-side */
    display: block; /* Change the display to block, for responsive reasons (see below) */
    color: white; /* White text color */
    text-align: center; /* Center the text */
    padding: 10px 50px; /* Add some padding */
    text-decoration: none; /* Remove underline */
  }
  /* td {
           
            border: 1px solid blak;
        }
   */
        th,
        td {
            font-weight: bold;
            border: 1px solid white;
            padding: 4px;
            text-align: center;
        }
  
        td {
            font-weight: lighter;
        }
        .container{
            text-align: center;
        }
</style>
<body>
    <img class="bg" src="fallback.jpg" alt="UEFA">
    <div class="container">
        <img class="img" src="logo2.png" alt="logo" height="80px">
    </div>
    <div class="navbar">
        <a href="Home.php">Home</a>
        <a href="fixtures.php">Fixtures</a>
        <a href="result.php">Results</a>
        <a href="group.php">Groups</a>
        <a href="transferlogs.php">Transferlogs</a>
        <a href="team.php">Teams</a>
        <a href="player.php">Player</a>
        <a href="league_standing.php">League Standing</a>
        <a href="about.php">About</a>
    </div>
    <div class="button" >
        <form action="playerfix.php" method="post" style="margin-bottom: 10px;">
            <button class="add">Fixtures of Player</button> 
        </form>
        <form action="fixdata.php" method="post" style="margin-bottom: 10px;">
            <button class="add">Data of Match</button> 
        </form>
    </div>
    <div class="league_stand">
    <section class="league_stand">
        <!-- <h1>League Standing</h1> -->
        <!-- TABLE CONSTRUCTION-->
        <table>
            <tr>
               <th>Match ID</th>
                <th>Home</th>
                <th>Away</th>
                <th>Group</th>
                <th>Date</th>
                <th>Time</th>
                <th>Goal T1</th>
                <th>Goal T2</th>
                <th>Stadium</th>
            </tr>
            <!-- PHP CODE TO FETCH DATA FROM ROWS-->
            <?php   // LOOP TILL END OF DATA 
            $servername = "localhost";
            $username = "root";
            $password = "";
            $db="football";
            
            // Create connection
            $con = mysqli_connect($servername, $username, $password,$db);
            $sql="select * from fixtures;";
            $result = mysqli_query($con,$sql);
                while($rows=$result->fetch_assoc())
                {
             ?>
            <tr>
                <!--FETCHING DATA FROM EACH 
                    ROW OF EVERY COLUMN-->
                <td><?php echo $rows['Match_ID'];?></td>
                <td><?php echo $rows['Home'];?></td>
                <td><?php echo $rows['Away'];?></td>
                <td><?php echo $rows['Grp'];?></td>
                <td><?php echo $rows['Date'];?></td>
                <td><?php echo $rows['Time'];?></td>
                <td><?php echo $rows['Goal_T1'];?></td>
                <td><?php echo $rows['Goal_T2'];?></td>
                <td><?php echo $rows['Stadium'];?></td>
            </tr>
            <?php
                }
             ?>
        </table>
    </section>
    
    </div>
    

</body>
</html> 